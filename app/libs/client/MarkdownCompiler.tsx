      ...
      <tbody>
        {Children.toArray(children).map((tr, rows) =>
          isValidElement(tr) && tr.type === "tr" ? (
            <tr key={tr.key} {...(tr.props as object)}>
              {Children.toArray(
                ...
                isValidElement(td) && td.type === "td" ? (
                  rows === 0 ? (
                    <th
                      key={td.key}
                      {...(typeof td.props === "object" ? td.props : {})}
                      style={{ textAlign: node.align?.[cols] ?? undefined }}
                    />
                  ) : (
                    <td
                      key={td.key}
...
function ReactCompiler(
  ...
  const reactNode = (vnode: VNode): React.ReactNode => {
    ...
    const markdownContent =
      components?.[vnode.type as keyof typeof components] ??
      defaultComponents[vnode.type as keyof typeof defaultComponents];
